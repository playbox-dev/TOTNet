...
from model.motion_model_v3 import build_motion_model_light
...
def train_one_epoch(train_loader, model, optimizer, loss_func, scaler, epoch, configs, logger):
    ...
    for batch_idx, (batch_data, (_, labels, visibiltity, status)) in enumerate(tqdm(train_loader)):
        ...
        with torch.autocast(device_type='cuda'):
            output_heatmap, cls_score, previous_feature_map = model(batch_data, previous_feature_map) # output in shape ([B, W],[B, H]) if output heatmap
...
def evaluate_one_epoch(val_loader, model, loss_func, epoch, configs, logger):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (batch_data, (_, labels, visibility, status)) in enumerate(tqdm(val_loader)):
            ...
            with torch.autocast(device_type='cuda'):
                output_heatmap, cls_score, previous_featuremap = model(batch_data, previous_featuremap) # output in shape ([B, W],[B, H]) if output heatmap, just raw logits
