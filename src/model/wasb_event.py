...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, pad='same', stride=1, bias=True):
        super().__init__()
...
class EventNet(nn.Module):
    def __init__(self, hidden_dims=[512, 256], num_frames=5, dropout=0.1, output_dim=2):
        super(EventNet, self).__init__()
        self.conv1 = ConvBlock(in_channels=num_frames*3, out_channels=32)
        ...
        self.pool3 = nn.AdaptiveMaxPool2d(output_size=(2, 2))
        # self.conv8 = ConvBlock(in_channels=512, out_channels=512)
        # self.conv9 = ConvBlock(in_channels=512, out_channels=512)
        # self.conv10 = ConvBlock(in_channels=512, out_channels=512)
        # self.pool4 = nn.AdaptiveMaxPool2d(output_size=(2, 2))
        # shape will be [2, 2, 512]
...
class HRNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.event_network = EventNet(num_frames=self._frames_in)
    ...
    def forward(self, x):
        event_prob = self.event_network(x)
        event_prob = self.sigmoid(event_prob)
        ...
        return (y_out[0][0], y_out[0][1]), event_prob
    ...
    def init_weights(self, pretrained='',):
        ...
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                ...
                nn.init.xavier_uniform_(m.weight)  # Better weight initialization
