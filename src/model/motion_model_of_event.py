...
class ClassificationHead(nn.Module):
    def __init__(self, input_dim, hidden_dims, output_dim, dropout=0.1):
        ...
        self.conv_block1 = ConvBlock(in_channels=512, out_channels=512, kernel_size=1, pad='same')
        self.conv_block2 = ConvBlock(in_channels=512, out_channels=512, kernel_size=1, pad='same')
        self.pool1 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.conv_block3 = ConvBlock(in_channels=512, out_channels=512, kernel_size=1, pad='same')
        self.conv_block4 = ConvBlock(in_channels=512, out_channels=512, kernel_size=1, pad='same')
        # shape [512, 18, 32]
...
class TemporalConvNet(nn.Module):
    def __init__(self, input_shape=(288, 512), spatial_channels=64, num_frames=5):
        ...
        self.classification_net = ClassificationHead(input_dim=2048, hidden_dims=[1024, 512], output_dim=2)
    ...
    def _init_weights(self):
        for module in self.modules():
            if isinstance(module, (nn.Conv3d, nn.Conv2d, nn.Conv1d)):
                ...
                nn.init.xavier_uniform_(module.weight)  # Better weight initialization
                if module.bias is not None:
                    nn.init.constant_(module.bias, 0)
    ...
    def forward(self, x):
        ...
        classification_out = self.classification_net(x)
        classification_out = self.sigmoid(classification_out)
        ...
        return (horizontal_heatmap, vertical_heatmap), classification_out
